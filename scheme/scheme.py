...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            proc = procedure.apply_macro(rest, env)
            return scheme_eval(proc, env)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if not test and clause.first != 'else':
                return
            else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    if bindings is nil:
        env.make_child_frame(nil, nil)
    current_pair = bindings.first
    check_form(current_pair, 2, 2)
    formals = Pair (current_pair.first, nil)
    vals = Pair(scheme_eval(current_pair.second.first, env),nil)
    formal_pointer = formals
    vals_pointer = vals
    bindings = bindings.second
    while bindings is not nil:
        current_pair = bindings.first
        check_form(current_pair, 2, 2)
        formal_pointer.second = Pair (current_pair.first, nil)
        vals_pointer.second = Pair(scheme_eval(current_pair.second.first, env),nil)
        formal_pointer = formal_pointer.second
        vals_pointer = vals_pointer.second
        bindings = bindings.second
    check_formals(formals)
    ...
    env.make_child_frame(formals, vals)
    #check_form(bindings, min, max=float('inf'))
    #check_formals(formals)
    # END PROBLEM 15
